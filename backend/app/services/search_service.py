...
Flow:
  1. Use Tavily to search the web for professors in the student's field.
  2. Use an LLM (via gorouter.app, paid — avoids Gemini free-tier limits)
     to extract structured professor info from raw search results.
  3. Embed each professor's research summary (Voyage AI).
  4. Store professors + embeddings in Supabase.
  5. Run a pgvector similarity search to rank professors against the
     student's own profile embedding.
...
async def _call_llm(prompt: str) -> str:
    """
    Call the LLM via gorouter.app (paid, OpenAI-compatible). Returns
    the raw text response, or an empty string on failure.
    """
    try:
        ...
        print(f"[search_service] LLM call failed: {e}")
        return ""
...
async def extract_professors_with_gemini(raw_results: list[dict]) -> list[dict]:
    """
    Use the LLM to turn raw Tavily search results (titles + content
...
"""
    ...
    text = _strip_json_fences(await _call_llm(prompt))
    if not text:
        return []
...
{combined_snippets}
"""
    candidate = (await _call_llm(prompt)).strip()
